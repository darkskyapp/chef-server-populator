...
describe 'chef-server-populator::restore' do
  ...
  let(:chef_run) do
    ChefSpec::SoloRunner.new do |node|
      node.default['chef_server_populator']['restore']['local_path'] = restore_path
    end.converge(described_recipe)
  end

  context 'when provided a URL for the database dump' do
    it 'downloads the remote file' do
      chef_run.node.default['chef_server_populator']['restore']['file'] = 'https://www.example.com/restore.dump'
  ...
  context 'when provided a URL for the tarball' do
    it 'downloads the remote file' do
      chef_run.node.default['chef_server_populator']['restore']['data'] = 'https://www.example.com/restore.tgz'
      chef_run.converge(described_recipe)
